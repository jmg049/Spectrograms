...
legacy \code{mdct\_f32} and \code{imdct\_f32} entry points are retained
as thin wrappers over the generic \code{mdct} and \code{imdct}
instantiated at \code{f32}.
...
machine-learning pipelines frequently train in single precision, so
producing features directly in \code{f32} avoids a wasteful conversion
at the boundary. Third, a planned \code{no\_std}/embedded build targets
devices whose floating-point units are single-precision, where
\code{f64} arithmetic is emulated and therefore costly. Double
precision nonetheless remains the default, as it is the right choice
for offline analysis where numerical headroom matters
(\cref{subsec:precision-f64}).

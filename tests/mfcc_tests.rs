...
fn test_mfcc_basic() {
    ...
    let mfcc_params = MfccParams::new(13).unwrap();

    let result = mfcc(&samples, &stft, sample_rate, 40, &mfcc_params).unwrap();

    // Check dimensions
    assert_eq!(result.n_coefficients(), 13);
...
#[test]
fn test_mfcc_params_validation() {
    // Valid parameters
    assert!(MfccParams::new(13).is_ok());
    assert!(MfccParams::new(20).is_ok());

    // Invalid: zero n_mfcc
    assert!(MfccParams::new(0).is_err());
}
...
fn test_mfcc_with_c0() {
    ...
    // Without C0
    let mfcc_params = MfccParams::new(13).unwrap();
    let result1 = mfcc(&samples, &stft, sample_rate, 40, &mfcc_params).unwrap();
    assert_eq!(result1.n_coefficients(), 13);

    // With C0
    let mfcc_params = MfccParams::new(13).unwrap().with_c0(true);
    let result2 = mfcc(&samples, &stft, sample_rate, 40, &mfcc_params).unwrap();
    assert_eq!(result2.n_coefficients(), 13);
...
fn test_mfcc_with_liftering() {
    ...
    // Without liftering
    let mfcc_params1 = MfccParams::new(13).unwrap();
    let result1 = mfcc(&samples, &stft, sample_rate, 40, &mfcc_params1).unwrap();

    // With liftering (L=22 is common)
    let mfcc_params2 = MfccParams::new(13).unwrap().with_lifter(22);
    let result2 = mfcc(&samples, &stft, sample_rate, 40, &mfcc_params2).unwrap();
...
fn test_mfcc_different_n_coefficients() {
    ...
    let mfcc_params = MfccParams::new(20).unwrap();
    let result = mfcc(&samples, &stft, sample_rate, 40, &mfcc_params).unwrap();
    assert_eq!(result.n_coefficients(), 20);

    // Test with 7 coefficients
    let mfcc_params = MfccParams::new(7).unwrap();
    let result = mfcc(&samples, &stft, sample_rate, 40, &mfcc_params).unwrap();
    assert_eq!(result.n_coefficients(), 7);
}
...
fn test_mfcc_speech_defaults() {
    ...
    let stft = StftParams::new(400, 160, WindowType::Hanning, true).unwrap();
    let mfcc_params = MfccParams::speech_standard().unwrap();
...
fn test_mfcc_decorrelation() {
    ...
    if n_frames > 0 {
        ...
        let mut avg_energies = vec![0.0; result.n_coefficients()];
        for coef in 0..result.n_coefficients() {

...
fn test_cqt_integration_basic() {
    ...
    let stft = StftParams::new(nzu!(16384), nzu!(4096), WindowType::Hanning, false).unwrap();
    ...
    // Basic checks
    assert_eq!(spectrogram.n_bins().get(), 84); // 12 * 7 = 84 bins
    ...
    for bin in 0..spectrogram.n_bins().get() {
        let energy: f64 = (0..spectrogram.n_frames().get())
...
fn test_cqt_params_validation() {
    // Valid parameters
    assert!(CqtParams::new(nzu!(12), nzu!(7), 32.7).is_ok());

    // Invalid: negative f_min
    assert!(CqtParams::new(nzu!(12), nzu!(7), -10.0).is_err());

    // Invalid: zero f_min
    assert!(CqtParams::new(nzu!(12), nzu!(7), 0.0).is_err());

    // Invalid: infinite f_min
    assert!(CqtParams::new(nzu!(12), nzu!(7), f64::INFINITY).is_err());
}

#[test]
fn test_cqt_num_bins() {
    let cqt = CqtParams::new(nzu!(12), nzu!(7), 32.7).unwrap();
    assert_eq!(cqt.num_bins().get(), 84); // 12 * 7
    let cqt = CqtParams::new(nzu!(24), nzu!(5), 20.0).unwrap();
    assert_eq!(cqt.num_bins().get(), 120); // 24 * 5
}

#[test]
fn test_cqt_frequencies() {
    let cqt = CqtParams::new(nzu!(12), nzu!(1), 100.0).unwrap();

    // Should have 12 bins (one octave)
    assert_eq!(cqt.num_bins().get(), 12);
    ...
    // Specifically: 100 * 2^(11/12) \approx 189.0 Hz
...
fn test_cqt_with_power_amplitude() {
    ...
    let cqt_params = CqtParams::new(nzu!(12), nzu!(5), 50.0).unwrap();
...
fn test_cqt_frame_computation() {
    ...
    assert_eq!(frame.len().get(), 60); // 12 * 5 = 60 bins
...
fn test_cqt_output_shape() {
    let sample_rate = 16000.0;
    let signal_length = nzu!(16000);

    let cqt_params = CqtParams::new(nzu!(12), nzu!(6), 40.0).unwrap();
    let stft = StftParams::new(nzu!(8192), nzu!(2048), WindowType::Hanning, false).unwrap();
    let params = SpectrogramParams::new(stft, sample_rate).unwrap();
